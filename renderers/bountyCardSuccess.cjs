...
async function createBountySuccessCard(options) {
  ...
  // New top box height (70%)
  const infoBoxHeight = leftHeight * 0.70;
  const infoBoxY = leftY;

  // Bottom COMPLETED box height reduced
  ...
  // TEXT (same layout as active-card)
  const LABEL_FONT = 60;
  const LINE_GAP = 75;
  const padX = 60;
  ...
  lines.push({ label: "Trainer:", value: username });
  lines.push({ label: "Rank:", value: rankName });
  ...
  lines.push({ label: "Reward:", value: rewardLabel });
  ...
  // SPRITES (full 1–3 logic from cardRenderer)
  ...
  if (spriteCount === 1) {
    const totalW = spriteSize;
    spriteXs = [rightX + (spriteRowWidth - totalW) / 2];
